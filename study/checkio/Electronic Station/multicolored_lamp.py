...
# Step through the colors
    def step(self):
        self.state = self.state+1 if self.state < len(self.inputs) else 1
        return self.inputs[self.state-1]

# Call step subrouting and return its result
    def light(self, inputs=('Green', 'Red', 'Blue', 'Yellow')):
        self.inputs = inputs
        return self.step()


class Lamp(StateMachine):
    startState = 0
