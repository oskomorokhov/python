...
def sun_angle(time_):
    ...
    return (datetime.combine(date.today(), time(int(time_[:2]), int(time_[3:]))) - datetime.combine(date.today(), time(6, 0))).seconds/60*(180/(12*60)) if datetime.combine(datetime.today(), time(6, 0)) <= datetime.combine(datetime.today(), time(int(time_[:2]), int(time_[3:]))) <= datetime.combine(datetime.today(), time(18, 0)) else "I don't see the sun!"
