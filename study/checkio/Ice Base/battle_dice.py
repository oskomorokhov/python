...
# https://py.checkio.org/mission/battle-dice/
...
def battle_probability(dice_description, units_one, units_two):
    ...
    while units_one > 0 and units_two > 0:
        ...
        roll1 = random.choice(dice_description)
        roll2 = random.choice(dice_description)

        print(roll1, roll2)

        losses_one = roll2.count(
            'A') - roll1.count('D') if roll2.count('A')-roll1.count('D') > 0 else 0
        losses_two = roll1.count(
            'A') - roll2.count('D') if roll1.count('A')-roll2.count('D') > 0 else 0

        print("l", losses_one, losses_two)

        units_one = units_one-losses_one
        units_two = units_two-losses_one

        print("u", units_one, units_two)

        c += 1

    print("c", c)

    if units_one == 0 and units_two == 0:
        return 0.0000
...
if __name__ == '__main__':
    ...
    assert(almost_equal(battle_probability(
        ['AA', 'A', 'D', 'DD'], 3, 4), 0.0186)), "You can win"
