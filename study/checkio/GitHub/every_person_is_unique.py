...
class Person:
    ...
    def name(self):
        return self.vars['first_name']+' '+self.vars['last_name']

    def age(self):
        return (datetime.strptime("01.01.2018", "%d.%m.%Y")-datetime.strptime(self.vars['birth_date'], "%d.%m.%Y")).days//365

    def work(self):
        return "{} a {}".format('He is' if self.vars['gender'] == 'male' else 'She is' if self.vars['gender'] == 'female' else 'Is', self.vars['job'])

    def money(self):
        return ''.join([' '+x if (i+1) % 3 == 0 else x for i, x in enumerate(str(self.vars['working_years']*self.vars['salary']*12)[::-1])][::-1]).strip()

    def home(self):
        return "Lives in {}, {}".format(self.vars['city'], self.vars['country'])
