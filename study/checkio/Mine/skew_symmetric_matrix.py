...
def checkio(matrix):
    ...
    nAT = [list(x) for x in zip(*matrix)]

    for x, q in enumerate(matrix):
        for y, z in enumerate(q):
            matrix[x][y] = -z
