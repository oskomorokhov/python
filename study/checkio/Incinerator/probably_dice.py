...
def probability(dice_number, sides, target):
    ...
    possibilities = sides**dice_number

    die = range(1, sides+1)
    # print("die",list(die))
    dice = [range(1, sides+1)]*dice_number

    combinations(die, dice_number, target)

    #print("final s",s)

    chances = s/possibilities

    print(round(chances, 4))
    return round(chances, 4)


def combinations(die, dice_number, target, c=1):
    ...
    if c < dice_number:
        ...
        for i in die:
            ...
            if sum(l) >= target:
                l.pop()
                #print("exceeded target, pop+break",l)
                break
            #print("l after append",l)
            #print("recur begin")
            combinations(die, dice_number, target, c)
        # print("final")
        if len(l) > 0:
            l.pop()
            return
        ...
        #print("depth",c,"== dice_number, no recur")

        for i in die:

            l.append(i)
            #print("else l after append",l)
            if len(l) == dice_number:
                if sum(l) == target:
                    #print("bingo! s+=1")
                    s += 1
                    l.pop()
                    #print("bingo pop+break",l)
                    break
                elif sum(l) > target:
                    l.pop()
                    #print("exceeded target, pop+break",l)
                    ...
                    l.pop()
                    #print("wrong sum pop, try next",l)
        else:
            if len(l) > 0:
                l.pop()
                #print("loop end pop",l)
        if len(l) > 0:
            l.pop()
            #print("return to prev level pop",l)
        return
